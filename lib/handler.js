...
handler.products._method.put = function (data, callback) {
  ...
  if (tokenId && phone && product_name) {
    handler.authenticateUser(tokenId, phone, function (bool) {
      if (bool) {
        if (price || weight || image || category) {
          _data.read("products", product_name, function (err, data) {
            if (!err && data && data.length > 0) {
              var dataObj = helper.parseObject(data);
              if (price) {
              ...
              if (category && category != dataObj.category.name) {
                //remove current product details from category
                handler.removeAssociatedProductsFromCategory(
                  category,
                  product_name,
                  done
                );
                //get new category details
                _data.read("category", category, function (err, catData) {
                  if (!err && catData) {
                    dataObj.category = helper.parseObject(catData);
                    callback(200, {
                      success: "category updated on new product",
                    });
                  } else {
                    callback(400, {
                      Error: "could not update category on product",
                    });
                  }
                });
              }
              _data.update("products", product_name, function (err) {
                if (!err) {
                  callback(200, { success: "record updated" }, "json");
                } else {
                  callback(400, {
                    Error: "could not update record for products",
                  });
                }
              });
            } else {
        ...
        _data.read("products", product_name, function (err, data) {
          if (!err && data) {
            ...
            _data.delete("products", product_name, function (err) {
              if (!err) {
                callback(200, { success: "product deleted" }, "json");
              } else {
                callback(400, { Error: "could not delete product" }, "json");
              }
            });
          } else {
            callback(404, { Error: "no matching record for product" }, "json");
          }
        });
      } else {
        callback(403, { Error: "un-authorized" }, "json");
...
handler.removeAssociatedProductsFromCategory = function (
  dataObj,
  product_name,
  done
) {
  _data.read("categories", dataObj.category.name, function (err, catData) {
    if (!err && catData && catData.length > 0) {
      var parsedCatData = helper.parseObject(catData);
      delete parsedCatData.products[product_name];
      //update categories
      _data.update(
        "categories",
      ...
      callback(400, { Error: "could not read specified category" }, "json");
    }
  });
  done();
};
