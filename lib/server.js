...
server.serverLogic = function (res, req) {
  ...
  if (trimmedPath.indexOf("public/") > -1) {
    chosenHandler =
      typeof router["public"] !== "undefined"
        ? router["public"]
        : handler.notFound;
  }
  ...
  req.on("end", function () {
    ...
    //handle any error that may  kill the thread
    try {
      chosenHandler(data, function (statusCode, payload, content_type) {
        server.chosenHandlerLogic(
          res,
          method,
          trimmedPath,
          statusCode,
...
server.chosenHandlerLogic = function (
  res,
  method,
  trimmedPath,
  statusCode,
  ...
  var content_type = typeof content_type == "string" ? content_type : "json";
  ...
  //if status code is 200, then log  it to terminal in green else log in read
  if (statusCode == 200) {
    debug(
      "\x1b[32m%s\x1b[0m",
...
var router = {
  ...
  "api/sample-error": handler.sampleError,
  "api/checks": handler.checks,
  "api/categories": handler.categories,
  "api/products": handler.products,
